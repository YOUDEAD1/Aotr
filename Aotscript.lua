...
getgenv().ExunysDeveloperAimbot = {
	DeveloperSettings = {
		...
		TeamCheckOption = "Team", -- تغيير إلى "Team" بدلاً من "TeamColor" للتحقق من الفريق
		RainbowSpeed = 1
	},
 
	Settings = {
		Enabled = true,
		TeamCheck = true, -- تفعيل التحقق من الفريق
		AliveCheck = true,
		WallCheck = false,
	...
	if not Environment.Locked then
		...
		for _, Value in next, GetPlayers(Players) do
			...
			if Value ~= LocalPlayer and not tablefind(Environment.Blacklisted, __index(Value, "Name")) and Character and FindFirstChild(Character, LockPart) and Humanoid then
				...
				-- تحسين التحقق من الفريق
				if Settings.TeamCheck and __index(Value, TeamCheckOption) == __index(LocalPlayer, TeamCheckOption) then
					continue -- تخطي الحليف إذا كان في نفس الفريق
				end
...
local Load = function()
	...
	ServiceConnections.InputBeganConnection = Connect(__index(UserInputService, "InputBegan"), function(Input)
		...
		-- إضافة مفتاح لتفعيل/إيقاف السكريبت بالكامل (F)
		if Input.KeyCode == Enum.KeyCode.F then
			Settings.Enabled = not Settings.Enabled
			print("Aimbot " .. (Settings.Enabled and "Enabled" or "Disabled"))
			if not Settings.Enabled then CancelLock() end
		end
	end)
