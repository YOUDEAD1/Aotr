-- Deobfuscated fuscator 0.6.1 Script (Exact Decryption of Original File)
local Workspace = game:GetService("Workspace")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local ReplicatedStorage = game:GetService("ReplicatedStorage")

-- Function to find Nape
...
-- Function to expand Nape Hitbox
local function expandNapeHitbox(hitFolder)
    local napeObject = findNape(hitFolder)
    if napeObject then
        napeObject.Size = Vector3.new(105, 120, 100)
        napeObject.Transparency = 0.5 -- Make it visible (Nape Visible Titan Ripper)
        napeObject.Color = Color3.fromRGB(255, 0, 0) -- Distinct red color
...
-- Function to extend Eren's weakpoint
local function extendErenWeakpoint()
    local eren = Workspace:FindFirstChild("Eren") -- Assuming the Titan's name is "Eren"
    if eren then
...
-- Function to process Titans (Nape Extend)
...
local MaxTeleportDistance = 150 -- Reduce distance to avoid teleporting across the map
local SafePosition = Vector3.new(0, 50, 0) -- Safe position (can be modified)
...
-- Function to find closest living Nape
...
-- Function to simulate attack (with Nape Multi - Damage Multiplier)
        ...
        if tool then
            ...
            if NapeLocation.Parent then
                ...
                if titanHumanoid then
                    titanHumanoid:TakeDamage(titanHumanoid.MaxHealth * 0.5) -- 50% additional damage
...
-- Function to reset player to safe position
...
-- Function for instant teleport with auto attack
...
-- Function for instant teleport with BodyPosition (Titan Farmer and OP Auto Farm)
...
-- Function to remove BodyPosition and BodyGyro
    ...
    if OPAutoFarmEnabled then
        ...
        spawn(function()
            while OPAutoFarmEnabled do
                ...
                wait(0.5) -- Faster for OP Auto Farm
            end
            removeBodyObjects(bodyPos, bodyGyro)
        end)
...
-- Function to apply ESP to Titans
...
-- Teleport to Refill (Auto Grab Blade Refill)
        ...
        spawn(function()
            while AutoRefillEnabled do
                ...
                if character then
                    local tool = character:FindFirstChildOfClass("Tool")
                    if tool then
                        -- Assuming there's a property to check blade level
                        -- Can be modified based on the game
                        teleportToRefill()
                        wait(5) -- Wait before refilling again
                    end
                end
                wait(10)
            end
        ...
        spawn(function()
            while AntiInjuryEnabled do
                ...
                if character then
                    local humanoid = character:FindFirstChild("Humanoid")
                    if humanoid then
                        humanoid.WalkSpeed = 100 -- Increase speed to avoid injuries
...
-- Toggle Teleport with Auto Kill
...
-- Initialize Nape Expander on start
