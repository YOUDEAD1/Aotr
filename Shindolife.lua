-- حذف الجزء المتعلق بالتحقق من المفتاح وجعل التحميل مباشرًا
...
-- تعريف الدوال والمتغيرات الأساسية للواجهة
...
if not isfile("nexlib/config.cfg") then writefile("nexlib/config.cfg", tostring(q:JSONEncode({}))) end

-- دالة لتحريك الإطارات
        ...
        s.InputBegan:Connect(function(y)
            if y.UserInputType==Enum.UserInputType.MouseButton1 then 
                ...
                y.Changed:Connect(function()
                    if y.UserInputState==Enum.UserInputState.End then u=false end
                end)
            end
        end)
...
function k:Window(D)
    ...
    r(V,O)
    l.InputBegan:Connect(function(Y,Z)
    ...
    function _:Tab(D)
        ...
        a1.MouseButton1Click:Connect(function()
            for i,j in next,S:GetChildren() do 
                if j.Name=="TabBtn" then 
                    m:Create(j,TweenInfo.new(.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=0.4}):Play()
                end 
            end
            for i,j in next,R:GetChildren() do 
                if j.Name=="SectionHolder1" then j.Visible=false end 
            end
            for i,j in next,R:GetChildren() do 
                if j.Name=="SectionHolder2" then j.Visible=false end 
            end
            a2.Visible=true
            a5.Visible=true
            m:Create(a1,TweenInfo.new(.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=0}):Play()
        end)
        local a8={}
        function a8:Section(D)
            ...
            local aa=0
            local ab=0
            for i,j in next,a2:GetChildren() do if j.Name=="Section" then aa=aa+1 end end
            for i,j in next,a5:GetChildren() do if j.Name=="Section" then ab=ab+1 end end
            if aa==0 and ab==0 then a9=a2 elseif aa==ab then a9=a2 else a9=a5 end
            ...
            function a8:Toggle(D,an,aj)
                ...
                ao.MouseEnter:Connect(function() ap.BorderSizePixel=1 end)
                ao.MouseLeave:Connect(function() ap.BorderSizePixel=0 end)
                ao.MouseButton1Click:Connect(function()
                    M=not M
                    if M==true then 
                        m:Create(as,TweenInfo.new(.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=0}):Play()
                    else 
                        m:Create(as,TweenInfo.new(.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=0.4}):Play()
                    end
                    pcall(aj,M)
                end)
                if M then 
                ...
                ac.Size=UDim2.new(1,-2,0,ai.AbsoluteContentSize.Y+24)
                coroutine.wrap(function() while wait() do ap.BorderColor3=k.accentclr if M==true then ap.BackgroundColor3=k.accentclr else ap.BackgroundColor3=Color3.fromRGB(38,38,38) end end end)()
            end
            function a8:Slider(D,F,at,aj)
                local au={}
                local u=false
                ...
                local function aC(aD)
                    ...
                    local aF=at[4]and at[2]/((at[2]-at[1])/(at[4]*4))or at[2]>=50 and at[2]/((at[2]-at[1])/4)or at[2]>=25 and at[2]/((at[2]-at[1])/2)or at[2]/(at[2]-at[1])
                    local aG=UDim2.new(math.round(aE*at[2]/aF*4)/(at[2]/aF*4),0,1,0)
                    ay.Size=aG
                    local aH=math.round((aG.X.Scale*at[2]/at[2]*(at[2]-at[1])+at[1])*20)/20
                    aB.Text=tostring(aH)
                    if F==1 then aB.Text=tostring(aH) else aB.Text=D..": "..tostring(aH) end
                    pcall(aj,aH)
                end
                ax.InputBegan:Connect(function(y) if y.UserInputType==Enum.UserInputType.MouseButton1 then u=true end end)
                ax.InputEnded:Connect(function(y) if y.UserInputType==Enum.UserInputType.MouseButton1 then u=false end end)
                l.InputChanged:Connect(function(y) if u and y.UserInputType==Enum.UserInputType.MouseMovement then aC(y) end end)
                ac.Size=UDim2.new(1,-2,0,ai.AbsoluteContentSize.Y+24)
                coroutine.wrap(function() while wait() do ay.BackgroundColor3=k.accentclr end end)()
                function au:Change(aI)
                    aB.Text=tostring(aI and math.floor(aI/at[2]*(at[2]-at[1])+at[1])or 0)
                    if F==1 then aB.Text=tostring(aI and math.floor(aI/at[2]*(at[2]-at[1])+at[1])or 0) else aB.Text=D..": "..tostring(aI and math.floor(aI/at[2]*(at[2]-at[1])+at[1])or 0) end
            ...
            function a8:Dropdown(D,aJ,aK,aj)
                ...
                aO.MouseButton1Click:Connect(function()
                    if aT.Visible==false then 
                        for i,j in next,k.dropdownframes do if j.Name=="DropdownHolderFrame" then j.Visible=false end end
                        for i,j in next,k.dropdownframes do if j.Name=="Dropdown" then j.DropdownFrame.DropdownArrow.Rotation=0 end end
                        for i,j in next,k.colorpickerframes do if j.Name=="ColorpickerFrame" then j.Visible=false end end
                        for i,j in next,k.colorpickerframes do if j.Name=="Colorpicker" then j.ColorpickerColorFrame.BorderSizePixel=0 end end
                        aS.Rotation=180
                        aT.Visible=true 
                    else 
                        aS.Rotation=0
                ...
                for i,j in next,aJ do 
                    ...
                    aZ.MouseButton1Click:Connect(function() aT.Visible=false aR.Text=j aK=j pcall(aj,j) end)
                    coroutine.wrap(function() while wait() do a_.TextTransparency=typeof(aK)=="string" and aK==j and 0 or 1 a_.TextColor3=k.accentclr aZ.TextTransparency=typeof(aK)=="string" and aK==j and 1 or 0 aZ.BackgroundTransparency=typeof(aK)=="string" and aK==j and 0 or 1 aZ.BorderColor3=k.accentclr end end)()
                    aT.Size=UDim2.new(1,-16,0,aX.AbsoluteContentSize.Y+(#aW:GetChildren()-2)*2)
                end
                coroutine.wrap(function() while wait() do aO.BorderColor3=k.accentclr end end)()
            end
            function a8:Colorpicker(D,b0,aj)
                ...
                bd.MouseEnter:Connect(function() if bi.Visible==false then be.BorderSizePixel=1 end end)
                bd.MouseLeave:Connect(function() if bi.Visible==false then be.BorderSizePixel=0 end end)
                bd.MouseButton1Click:Connect(function()
                    if bi.Visible==false then 
                        for i,j in next,k.colorpickerframes do if j.Name=="ColorpickerFrame" then j.Visible=false end end
                        for i,j in next,k.colorpickerframes do if j.Name=="Colorpicker" then j.ColorpickerColorFrame.BorderSizePixel=0 end end
                        for i,j in next,k.dropdownframes do if j.Name=="DropdownHolderFrame" then j.Visible=false end end
                        for i,j in next,k.dropdownframes do if j.Name=="DropdownFrame" then j.DropdownBtn.DropdownFrame1.DropdownFrameIco.Rotation=0 end end
                        bi.Visible=true
                ...
                bm.InputEnded:Connect(function(y) if y.UserInputType==Enum.UserInputType.MouseButton1 then if bb then bb:Disconnect() end end end)
                ...
                bj.InputEnded:Connect(function(y) if y.UserInputType==Enum.UserInputType.MouseButton1 then if bc then bc:Disconnect() end end end)
                coroutine.wrap(function() while wait() do be.BorderColor3=k.accentclr end end)()
            end
            function a8:Bind(D,b0,aj)
                ...
                bt.MouseButton1Down:Connect(function()
                    ...
                    bv=l.InputBegan:Connect(function(y)
                        local bw=nil
                        pcall(function()
                            if not f(e,y.KeyCode) then bw=tostring(y.KeyCode) end
                            if f(d,y.UserInputType) and not bw then bw=d[y.UserInputType] end
                        end)
                    ...
                    delay(5,function() if bv then bv:Disconnect() bv=nil aj({}) bu.Text="NONE" end end)
                end)
            end
            function a8:Textbox(D,bx,by,aj)
                ...
                ac.Size=UDim2.new(1,-2,0,ai.AbsoluteContentSize.Y+24)
                bE.Changed:Connect(function(bF) if bF then if #bE.Text>0 then pcall(aj,bE.Text) end end end)
                bE.FocusLost:Connect(function(bF) if bF then if #bE.Text>0 then if by then bE.Text="" end end end end)
                bE:GetPropertyChangedSignal("Text"):Connect(function() if bx==true then bE.Text=bE.Text:gsub("%D+","") end end)
            end
            function a8:Label(D)
                local bG={}
                local bH=Instance.new("TextLabel")
                ...
                bH.TextXAlignment=Enum.TextXAlignment.Left
                ac.Size=UDim2.new(1,-2,0,ai.AbsoluteContentSize.Y+24)
                function bG:Change(bI) bH.Text=bI end
                return bG
            end
            return a8
        end
...
-- إعداد النافذة الرئيسية مع إشعار بدء التشغيل
local bJ = k:Window("SolixHub")
local bK = bJ:Tab("Main")
local bL = bK:Section("Info")
bL:Label("SolixHub Made By RelixEnd")
bL:Label("Script loaded successfully!")
k:Notification("SolixHub", "Welcome! Script loaded without key.", 3, nil)
